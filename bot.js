...
var bestsupports = ['518214960958930950','537147937583529994','','','','','','','','',''];// ايديات الي يقدرو يضيفو
client.on('message',async message => {
  if(!bestsupports.includes(message.auhtor.id)) return;
  if(message.content.startsWith(prefix + "blok")) {
...
client.on('message' , message => {
  if(!bestsupports.includes(message.auhtor.id)) return;
  if(message.content.startsWith(prefix + "blokserver")) {
    let args = message.content.split(' ').slice(1).join(' ');
    if(!isNaN(args)) return;
